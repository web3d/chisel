...
public class CoordAttractor {
    // one floater is marked as special for each attractor.  The index
    // of this array is the attractor offset, the value is the floater
    // offset in the floater array
    ...
    void attractFloater( int floaterOffset, float[] floaterEntity ) {
        ...
        for ( int i = 0; i < numberAttractors; i++ ) {
            ...
            attractorSelector.attemptAttractorSelection( i, 
                distanceBetween( floaterEntity, attractorEntity ));
        }
        attractorSelector.updatePreservedInfo( preservedFloaterOffsets,
            preservedFloaterDistances, floaterOffset );
