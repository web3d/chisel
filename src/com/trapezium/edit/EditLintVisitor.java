...
public class EditLintVisitor extends LintVisitor implements LineError {
    ...
    // have to track first and last error lines for positioning window
	...
	public void mergeErrors( FactoryResponseListener frl, BitSet errorBits, BitSet warningBits, BitSet nonconformanceBits ) {
		...
		// create keys into error list for fast lookup
		setErrorKeys();
			...
			if ( ei != null ) {
    			...
    			while ( ei != null ) {
        		    if (( errorBits != null ) && ( warningBits != null )) {
    	    	        // map error line into range 0-Slider.ErrorMarkCount
		...
		if ( frl != null ) {
		    frl.setLinePercent( -20 );  // hack to restore original error text
		}
	}
