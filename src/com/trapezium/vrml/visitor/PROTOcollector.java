...
public class PROTOcollector extends Visitor {
    // the list of PROTOs in the Scene
    Vector protoList;
    
    // the visited Scene is merged with a destination Scene
    // the visited Scene is the container for all objects visited
    Scene visitedScene;
    
    // the destination Scene is not visited, it is necessary only to
    // resolve name space conflicts
    Scene destinationScene;
    
    // bitmap of PROTOInstance locations
    BitSet protoInstanceLocations;
    // bitmap of PROTO locations
    BitSet protoLocations;
    // bitmap of USE locations
    BitSet useLocations;
    // bitmap of ROUTE locations
    BitSet routeLocations;
    int numberTokens;
    
    // PROTO name conflict resolution, key is old name, value is new name
    Hashtable protoMapTable;
    
    // DEF name conflict resolution, key is old name, value is new name
    Hashtable defMapTable;
    
    // PROTO lookup, key is PROTO token offset, value is PROTO
    Hashtable protoTable;
    
    // ROUTE lookup, key is ROUTE token offset, value is ROUTE
        ...
        if ( dun.isDEF() ) {
            ...
            if ( destinationScene.getDEF( defName ) == null ) {
                ...
                if ( dnf == null ) {
                    ...
                    // NOTE: we assume user supplied DEF name factory generates
                    // a name without a conflict
                    defMapTable.put( defName, s );
                    dun.setDEFName( s );
                    destinationScene.registerDEF( dun );
                    return;
